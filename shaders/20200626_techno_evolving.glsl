...
vec2 mainAudio( vec4 time ) {
  ...
  // -- bass ---------------------------------------------------------------------------------------
  {
    ...
    dest += vec2( amp ) * tri( lofi( time.y * n2f( -36.0 ), 0.0625 ) );
  }
  ...
  // -- acid ---------------------------------------------------------------------------------------
  {
    ...
    float freq = n2f( -48.0 + pattern[ int( time.y / ( 0.25 beat ) ) ] );
    float amp = 0.15;
  ...
  // -- pads ---------------------------------------------------------------------------------------
  {
    ...
    vec2 dice = random2( 0.81 * float( i ) );
      float freq = n2f( chord[ i ] - 12.0 + 0.5 * ( dice.x - 0.5 ) );
      wave += (
        wavetable( time.z * freq, mix( 1.4E-4, 2.4E-4, env ), 0.0004 * time.z + float( i ) )
      );
    }
    float amp = 0.06 * sidechain * env;
    dest += amp * wave;
  }

  // -- arp ----------------------------------------------------------------------------------------
  {
    float t = mod( mod( time.z, 8.0 beat ) - 0.25 beat, 0.75 beat );
    int iStep = int( ( mod( time.z, 8.0 beat ) - 0.25 beat ) / ( 0.75 beat ) );
    float pattern[11] = float[](
      0.0, 17.0, 14.0, 10.0,
      15.0, 14.0, 10.0, 7.0,
      14.0, 2.0, 5.0
    );
    float env = exp( -10.0 * t );
    float freq = n2f( pattern[ iStep % 11 ] - 12.0 + 12.0 * floor( mod( t * 80.0, 3.0 ) ) );
    vec2 wave = pwm( t * freq + vec2( 0.1, 0.6 ), vec2( 0.5 + t ) );
    float amp = 0.15 * mix( 0.4, 1.0, sidechain ) * env;
    dest += amp * wave;
  }
