...
vec2 mainAudio(vec4 time) {
  ...
  { // kick
    int pattern = 0x8888;
    { pattern = time.z < 60.0 * B2T ? 0x8888 : 0x809c; }
  ...
  if (time.z > 60.0 * B2T) { // snare909
    vec4 seq = seq16(time.y, 0xffff);
    float t = time.y > 2.0 * B2T
      ? mod(time.y, B2T / 6.0)
      : seq.y;

    float amp = 0.2 * mix(0.5, 1.0, smoothstep(0.0, 4.0 * B2T, time.y));
    ...
    dest += amp * mix(0.3, 1.0, sidechain) * tanh(4.0 * env * wave);
  }
