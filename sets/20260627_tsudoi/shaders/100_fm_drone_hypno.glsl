...
//      ████████████      @ 第七回 電子音楽ライヴの集い (VRChat), 2026-06-15
...
vec2 mainAudioDry(vec4 time) {
  ...
  //     - 4.0 * exp2(-t * 40.0)
  //   ));
  //   dest += 0.3 * mix(1.0, 0.4, MACRO3) * duck * tanh(2.0 * env * wave);
  // }
  ...
  //   dest += 0.3 * mix(1.0, 0.3, MACRO3) * mix(0.0, 1.0, duck) * env * wave;
  // }
  ...
  //   float decayk = float[](200.0, 100.0, 60.0, 60.0)[imod(int(floor(seq.s)), 4)];
  //   env *= exp2(-decayk * t);
