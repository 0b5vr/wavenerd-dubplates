...
vec2 mainAudioDry(vec4 time) {
  ...
  // }
  #define GET_IPROG(t) enableProgression * step(16.0 * B2T, mod(t, 32.0 * B2T))
  ...
  //   vec3 dice = hash3f(vec3(st, 20, 50));

  //   bool retrigger = dice.x < 0.2;
  //   if (retrigger) {
  //     st = st;

  //     float l = exp2(-floor(3.0 * dice.y)) * S2T;
  //     t = mod(time.x, l);
  //     q = l - t;
  //   }

  //   dice = hash3f(vec3(st, 40, 70));

  //   float env = smoothstep(0.0, 0.01, q);

  //   float freq = exp2(mix(6.0, 11.0, dice.x));
  //   float fall = exp2(mix(-6.0, -4.0, pow(dice.y, 6.0)));
  //   float phase = freq * exp(-t / fall) * fall;

  //   dice = hash3f(vec3(st, 21, 40));

  //   vec3 p = vec3(
  //     exp2(-1.0 + 4.0 * dice.x) * cis(TAU * phase),
  //     20.0 * dice.y + phase * exp2(mix(-8.0, 0.0, pow(dice.z, 3.0)))
  //   );
  ...
  { // arp
    ...
    repeat(i, 64) {
      ...
      phase *= exp2(detuned * 0.1 * (dice.xy - 0.5)); // detune
      phase += dice.yz;
