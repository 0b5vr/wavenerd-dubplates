...
vec2 mainAudioDry(vec4 time) {
  ...
  { // kick
    int pattern = isFillIn ? 0x8064 : 0x8888;
    vec4 seq = seq16(time.y, S2T, pattern);
    float t = seq.t;
    float q = seq.q;
    duck = min(
      duck,
  ...
  //   dest += 0.1 * env * mix(0.5, 1.0, duck) * tanh(8.0 * wave);
  // }
  ...
  //   dest += 0.2 * exp(-8.0 * t) * duck * tanh(sum);
  // }
  ...
  if (time.z > 48.0 * B2T) { // sweep
    ...
    osc += cheapnoise(256.0 * t);
    osc += cheapnoise(256.0 * (t + 0.003 * exp(-0.2 * t)));
    osc += cheapnoise(256.0 * (t + 0.005 * exp(-0.2 * t)));
  ...
  { // crash
    ...
    dest += 0.4 * env * mix(0.3, 1.0, duck) * tanh(8.0 * wave);
  }
  ...
  //   dest += sum * mix(0.0, 1.0, gate) * mix(0.2, 1.0, duck);
